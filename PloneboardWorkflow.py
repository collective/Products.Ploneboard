...
def setupPloneboardCommentWorkflow(wf):
    ...
    for p in (AccessContentsInformation, ViewBoard, EditComment, 
...
def setupPloneboardForumWorkflow(wf):
    ...
    for v in ('action', 'actor', 'comments', 'review_history', 'time'):
        wf.variables.addVariable(v)
    for p in (AccessContentsInformation, ViewBoard, AddConversation, RequestReview,
    ...
    sdef.setPermission(RequestReview,  1, (r_manager, r_anon, r_member, r_reviewer))
...
def setupPloneboardWorkflow(wf):
    ...
    for p in (AccessContentsInformation, ModifyPortalContent, AddPortalContent,
              ...
              AddConversation, AddComment, ManageConversation, ManageComment, ApproveComment):
        wf.addManagedPermission(p)
