...
class TestForum(ZopeTestCase.ZopeTestCase):
    ...
    def test_addForum(self):
        """
        Create new folder in home directory & check its basic properties and behaviour
        """
        
        board = self.plone.board

        self.assertEqual(len(board.contentValues('Forum')), 0)
        board.addForum('forum', 'title', 'description')
        self.assertEqual(len(board.contentValues('Forum')), 1)
        ...
        # Youpie
        Log(LOG_NOTICE, "Forum instanciated correctly !")
        
    def test_removeForum(self):
        """
        Create new folder in home directory & check its basic properties and behaviour
        """
        board = self.plone.board
        ...
        # Youpie
        Log(LOG_NOTICE, "Forum removed correctly !")
