...
class TestPloneboardConversation(PloneboardTestCase.PloneboardTestCase):
    ...
    def testAddMessage(self):
        ...
        self.failUnless(msg_id in [v.id for v in self.catalog(meta_type='PloneboardMessage', id=msg_id)])
    ...
    def test_delObject(self):
        ...
        self.failUnless(r.getId() in [v.id for v in self.catalog(meta_type='PloneboardMessage', id=r.getId())])
        self.assertEqual(conv.getNumberOfMessages(), 7)
        conv._delObject(r2.getId(), recursive=1)
        self.assertEqual(conv.getNumberOfMessages(), 3)
        # check if we delete conversation so we delete root message
        ...
        self.failIf(r.getId() in [v.id for v in self.catalog(meta_type='PloneboardMessage', id=r.getId())])
        self.failIf(msg.getId() in [v.id for v in self.catalog(meta_type='PloneboardMessage', id=msg.getId())])
