...
$Id: ATContentType.py,v 1.26 2004/06/20 18:45:55 tiran Exp $
""" 
__author__  = ''
...
class ATCTMixin(TemplateMixin):
    ...
    def initializeArchetype(self, **kwargs):
        ...
        try:
            ...
            if DEBUG and str(msg) not in ('mark_creation_flag', 'SESSION',):
                # XXX debug code
                raise
...
class ATCTFileContent(ATCTContent):
    ...
    def _setATCTFileContent(self, value, **kwargs):
        ...
        if self.isIDAutoGenerated(self.getId()):
            ...
            if not id and not clean_filename == self.getId() and clean_filename:
                aq_parent(self).manage_renameObject(self.getId(), clean_filename)
                #self._v_rename_to_filename = clean_filename
    ...
    def post_validate(self, REQUEST=None, errors=None):
        ...
        parent_ids = [ob.getId() for ob in parent.objectValues()]
        field  = self.getPrimaryField()
        ...
        if id and id not in parent_ids:
            # id available and id not used
            return
        
        if clean_filename in parent_ids or id in parent_ids:
            # failure
