...
security_schema = Schema((
   ...
   BooleanField('mail_me',
    ...
    LinesField('domains',
               ...
               widget=LinesWidget(
                   label='Domains',
                   label_msgid='label_member_domains',
...
#            sys.stdout.write('isValid, errors = %s\n' % (str(errors)))
    ...
    def setProperties(self, mapping=None, **kwargs):
        ...
        if mapping.has_key('last_login_time'):
            self.setLastLoginTime(mapping['last_login_time'])
...
#   user object exists, self._userInfo is None
    ...
    def register(self, auto=0):
        try:
            ...
            if not self.hasUser():
                ...
                _limitGrantedRoles(effective_roles, self, ('Member',))
                self.setRoles(roles)
                self._createUser()
    ...
    def _changeFieldsAfterRegistration(self):
        ...
        XXX This is somewhat of an ugly hack. Find a better way! (Probably
            needs AT support to really fix this.)
        """
    ...
    def getSchema(self):
        """return the class schema plus any custom schema"""
        mdc = getToolByName(self, 'portal_memberdata')
        effective_schema = self.schema
        custom_schema = mdc.getCustomSchema()
        if custom_schema:
            effective_schema += custom_schema
